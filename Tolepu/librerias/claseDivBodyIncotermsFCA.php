<?php
	class claseDivBodyIncotermsFCA {
		public static function show() {
			...
			$lblTipoContenedor	       = "Tipo de Contenedor:";
			...
			$fcaMaritimo="
				<tr>
					<td>
						".claseDivLogo::show("../../Framework/images/Tolepu/underConstruction.png","50%")."
					</td>
				</tr>
			";
			...
			$FCA=$viaEmbarqueId=="0000000502"?"":($viaEmbarqueId=="0000000493"?"":$fcaTerrestre);
		...
		public static function saveRecord()	{
			...
			$arrayFCA	 = json_decode($dataObjectIncoterms,true);
			...
			$puertoOrigenId		=	$arrayFCA["PuertoOrigenId"];
			$puertoDestinoId	=	$arrayFCA["PuertoDestinoId"];
			$tipoCargaId		=	$arrayFCA["TipoCargaId"];
			$tipoMercaderiaId	=	$arrayFCA["TipoMercaderiaId"];
			$descMercaderia		=	$arrayFCA["DescMercaderia"];
			$infoAdicional		=	$arrayFCA["InfoAdicional"];
			$cantidadContenedor =	str_replace(",",".",$arrayFCA["CantidadContenedor"]);
			$tipoContenedorId	=	$arrayFCA["TipoContenedorId"];
			$descContenedor		=	$arrayFCA["DescContenedor"];
			$peso				=	str_replace(",",".",$arrayFCA["Peso"]);
			$pesoVolumen		=	str_replace(",",".",$arrayFCA["PesoVolumen"]);
			$volumen			=	str_replace(",",".",$arrayFCA["Volumen"]);
			$bultos				=	str_replace(",",".",$arrayFCA["Bultos"]);
			$unidadId			=	$arrayFCA["UnidadId"];
			$pesoVolumenId		=	$arrayFCA["PesoVolumenId"];
			$volumenId			=	$arrayFCA["VolumenId"];
			$embalajeId			=	$arrayFCA["EmbalajeId"];
			$dimensionBultos	=	$arrayFCA["DimensionBultos"];
			$claseImo			=	$arrayFCA["ClaseImo"];
			$un					=	$arrayFCA["Un"];
			$proveedorName		=	$arrayFCA["ProveedorName"];
			$provRecogidaDir	= 	$arrayFCA["ProvRecogidaDir"];
			$provEntregaDir		= 	$arrayFCA["ProvEntregaDir"];
			$navieraId			= 	$arrayFCA["NavieraId"];
			$valorMercaderia	= 	str_replace(",",".",$arrayFCA["ValorMercaderia"]);
			$valorFlete			= 	str_replace(",",".",$arrayFCA["ValorFlete"]);
			$partidaArancelaria	= 	$arrayFCA["PartidaArancelaria"];
			$subPartidaArancelaria	= 	$arrayFCA["SubPartidaArancelaria"];
			$coloaderDestino	= 	$arrayFCA["ColoaderDestino"];
			$cotizacion			=	$arrayFCA["Cotizacion"];
			$requerimiento		=	$arrayFCA["Requerimiento"];
			...
			if ( $viaEmbarqueId == "0000000499"  ) //Terrestre
			{
				...
				{
					$mostrar="
						Debe registrar la Dirección de Entrega.
					";
					$arrayMail[0]["error"]=1;
					$arrayMail[0]["message"]=$mostrar;
					$stringJson=json_encode($arrayMail);
