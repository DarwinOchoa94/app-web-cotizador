<?php
	class claseDivBodyIncotermsDAP {
		public static function show() {
			...
			$DAP=$viaEmbarqueId=="0000000502"?$dapAereo:($viaEmbarqueId=="0000000493"?$dapMaritimo:$dapTerrestre);
		...
		public static function saveRecord() {
			...
			$arrayDAP	 = json_decode($dataObjectIncoterms,true);
			...
			$puertoOrigenId		=	$arrayDAP["PuertoOrigenId"];
			$puertoDestinoId	=	$arrayDAP["PuertoDestinoId"];
			$tipoCargaId		=	$arrayDAP["TipoCargaId"];
			$tipoMercaderiaId	=	$arrayDAP["TipoMercaderiaId"];
			$descMercaderia		=	$arrayDAP["DescMercaderia"];
			$infoAdicional		=	$arrayDAP["InfoAdicional"];
			$cantidadContenedor =	str_replace(",",".",$arrayDAP["CantidadContenedor"]);
			$tipoContenedorId	=	$arrayDAP["TipoContenedorId"];
			$descContenedor		=	$arrayDAP["DescContenedor"];
			$peso				=	str_replace(",",".",$arrayDAP["Peso"]);
			$pesoVolumen		=	str_replace(",",".",$arrayDAP["PesoVolumen"]);
			$volumen			=	str_replace(",",".",$arrayDAP["Volumen"]);
			$bultos				=	str_replace(",",".",$arrayDAP["Bultos"]);
			$unidadId			=	$arrayDAP["UnidadId"];
			$pesoVolumenId		=	$arrayDAP["PesoVolumenId"];
			$volumenId			=	$arrayDAP["VolumenId"];
			$embalajeId			=	$arrayDAP["EmbalajeId"];
			$dimensionBultos	=	$arrayDAP["DimensionBultos"];
			$claseImo			=	$arrayDAP["ClaseImo"];
			$un					=	$arrayDAP["Un"];
			$proveedorName		=	$arrayDAP["ProveedorName"];
			$provRecogidaDir	= 	$arrayDAP["ProvRecogidaDir"];
			$provEntregaDir		= 	$arrayDAP["ProvEntregaDir"];
			$navieraId			= 	$arrayDAP["NavieraId"];
			$valorMercaderia	= 	str_replace(",",".",$arrayDAP["ValorMercaderia"]);
			$valorFlete			= 	str_replace(",",".",$arrayDAP["ValorFlete"]);
			$partidaArancelaria	= 	$arrayDAP["PartidaArancelaria"];
			$subPartidaArancelaria	= 	$arrayDAP["SubPartidaArancelaria"];
			$coloaderDestino	= 	$arrayDAP["ColoaderDestino"];
			$cotizacion			=	$arrayDAP["Cotizacion"];
			$requerimiento		=	$arrayDAP["Requerimiento"];
			...
			else
			{
				if ( ( $peso > 0 ) && ( $unidadId == "" ) )
				{
					/*
					$mostrar="
						Debe seleccionar la Unidad de Peso de la carga.
					";
					$arrayMail[0]["error"]=1;
					$arrayMail[0]["message"]=$mostrar;
					$stringJson=json_encode($arrayMail);
				...
				if ( ( $volumen > 0 ) && ( $volumenId == "" ) )
				{
					/*
					$mostrar="
						Debe seleccionar la Unidad de Volúmen de la carga.
					";
					$arrayMail[0]["error"]=1;
					$arrayMail[0]["message"]=$mostrar;
					$stringJson=json_encode($arrayMail);
