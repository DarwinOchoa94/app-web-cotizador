...
function loadComboBoxV2(controlId, parametro, isCode = false) {
	var comboBox = document.getElementById(controlId);
	var empresa = document.body.getAttribute("empresa");
	empresa = 'Tolepu';

	if ((comboBox.getAttribute("focusinicial") == 0) || (comboBox.value == "")) {
		var purl = "./index.php?action=loadComboBoxV2&parametro=" + parametro + "&empresa=" + empresa + "&isCode=" + isCode;
		var content = getContent(purl);
...
function crearChecks() {
	$('#checkComparativas').change(function () {
		if (this.checked) {
			$("#cmbTipoTransporte").attr("multiple", "multiple");
		} else {
			$("#cmbTipoTransporte").removeAttr("multiple")
		}
		$('#cmbTipoTransporte').select2({
			minimumResultsForSearch: -1,
			maximumSelectionLength: 2,
			placeholder: "Seleccione...",
			ajax: {
				url: "./index.php?action=loadComboBoxSearch&parametro=TTR" + "&empresa=Tolepu" + "&isCode=false",
				type: "post",
				dataType: 'json',
				delay: 250,
	...
	$('#checkUrgente').change(function (event) {
...
function crearSelects() {
	...
	$('#cmbOrigen').select2({
		placeholder: "Seleccione...",
		ajax: {
			url: "./index.php?action=loadComboBoxSearch&parametro=ORIDES" + "&empresa=Tolepu" + "&isCode=false",
			type: "post",
			dataType: 'json',
			delay: 250,
